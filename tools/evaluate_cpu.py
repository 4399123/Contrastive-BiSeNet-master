...
class Metrics(object):
    ...
    def compute_metrics(self,):
        ...
        mprecision=macro_precision.nanmean()
        mrecall=macro_recall.nanmean()
...
def print_res_table(tname, heads, weights, metric, cat_metric,printlabels):
    ...
    cat_res = [
        ...
        [f'{printlabels[idx]}', ] + [f'{el:.6f}' for el in group]
        for idx, group in enumerate(zip(*cat_res))]
...
@torch.no_grad()
def eval_model(cfg, net,printlabels):
    ...
    macro_precision, macro_recall, mprecision, mrecall = [], [], [], []
    ...
    macro_precision.append(metrics['macro_precision'])
    macro_recall.append(metrics['macro_recall'])
    mprecision.append(metrics['mprecision'])
    mrecall.append(metrics['mrecall'])
    ...
    iou_heads, iou_content = print_res_table('iou', heads,
            weights, metric, cat_ious,printlabels)
    metric = dict(macro_f1=macro_f1, micro_f1=micro_f1)
    f1_heads, f1_content = print_res_table('f1 score', heads,
            weights, metric, f1_scores,printlabels)

    #####
    metric = dict(mprecision=mprecision)
    precision_heads, precision_content = print_res_table('precision', heads,
            weights, metric, macro_precision,printlabels)

    metric = dict(mrecall=mrecall)
    recall_heads, recall_content = print_res_table('recall', heads,
            weights, metric, macro_recall,printlabels)


    net.aux_mode = org_aux
    return iou_heads, iou_content, f1_heads, f1_content,precision_heads, precision_content,recall_heads, recall_content
...
def evaluate(cfg, weight_pth):
    ...
    # iou_heads, iou_content, f1_heads, f1_content = eval_model(cfg, net,printlabels)
...
def parse_args():
    ...
    parse.add_argument('--weight-path', dest='weight_pth', type=str,
                       default='../pt/X1/v1_blueface_vovnet.pt',)
    parse.add_argument('--config', dest='config', type=str,
            default='../configs/bisenetv1_blueface_vovnet.py',)
