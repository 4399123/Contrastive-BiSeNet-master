...
from lib.logger import setup_logger, print_log_msg,print_log_msgs,print_log_msgsV2
from tqdm import tqdm
from timm.optim import AdamW,AdamP,RAdam,Lookahead,AdaBelief
...
def parse_args():
    ...
    parse.add_argument('--config', dest='config', type=str,
            default='../configs/bisenetv1_blueface_convnextv2_pico_arcface.py',)
    parse.add_argument('--finetune-from', type=str, default=None,)
    parse.add_argument("--local_rank", type=int)
    parse.add_argument("--embedding", type=int,default=512)
...
def set_model(lb_ignore=255):
    ...
    net = model_factory[cfg.model_type](cfg.n_cats,args.embedding)
...
def set_optimizer(model):
    ...
    # )
    arcloss = losses.SubCenterArcFaceLoss(num_classes=len(printlabels), embedding_size=args.embedding,
                                          weight_regularizer=regularizers.RegularFaceRegularizer()).to('cuda')
    optim_opt=1
...
def get_alpha(epoch,eopchs):
    init_alpha=0.001
    final_alpha=1.0
    epoch=epoch+1
    eopchs=eopchs+1
    k=(final_alpha-init_alpha)/eopchs
    b=init_alpha-k
    alpha=k*epoch+b
    return alpha
...
def train(writer):
    ...
    ## optimizer
    optim,arcloss = set_optimizer(net)
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for it, (im, lb) in enumerate(dl):
            ...
            lb_bak = lb.reshape(-1, 1).squeeze()
            optim.zero_grad()
            with amp.autocast(enabled=cfg.use_fp16):
                logits, *logits_aux,feat_fuse = net(im)
                loss_pre = criteria_pre(logits, lb)
                loss_aux = [crit(lgt, lb) for crit, lgt in zip(criteria_aux, logits_aux)]

                loss = loss_pre + sum(loss_aux)
                arcfaceloss = arcloss(feat_fuse, lb_bak)
                alpha=get_alpha(epoch,cfg.max_epochs)
                arcfaceloss=alpha*arcfaceloss
                loss+=arcfaceloss

            arcfaceloss = arcfaceloss.item()
            ...
            # if (it + 1) % 200 == 0:
            if (it + 1) % gap == 0:
                print_log_msgsV2(epoch, cfg.max_epochs, it, len(dl), lr, loss_meter, loss_pre_meter,loss_aux_meters,arcfaceloss,writer)
