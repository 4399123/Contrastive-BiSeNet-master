...
parse.add_argument('--weight-path', type=str, default='../pt/2V22/v1_blueface_convnext_small.pt',)
...
for image in tqdm(images):
    ...
    # cv2.imwrite('./out/predicted_{}_{}'.format(labelid[1],labelid[2]), out)
    # pred = palette[out]
    # cv2.imwrite('./out/out1_{}.jpg'.format(n), pred)
    #
    # img=cv2.addWeighted(img1,0.7,pred,0.3,0)
    # cv2.imwrite('./out/{}'.format(basename), img)
    # n+=1
    ...
    pred = palette[out]
    cv2.imwrite('./out/mask_{}.jpg'.format(basename), pred)
    # cv2.imwrite('./out/mask_{}_{}'.format(labelid[1],labelid[2]), pred)
    ...
    base_img.save('./out/out_{}.jpg'.format(basename))
