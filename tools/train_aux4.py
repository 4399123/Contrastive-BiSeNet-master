...
def parse_args():
    ...
    parse.add_argument('--config', dest='config', type=str,
            default='../configs/bisenetv1_blueface_convnext_small_transpose_sep_atrous_aux4.py',)
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for it, (im, lb) in enumerate(dl):
            ...
            with amp.autocast(enabled=cfg.use_fp16):
                logits, *logits_aux,logits_aux8,logits_aux4= net(im)
                loss_pre = criteria_pre(logits, lb)
                loss_aux = [crit(lgt, lb) for crit, lgt in zip(criteria_aux, logits_aux)]
                loss_aux8 = criteria_pre(logits_aux8, lb) * 0.5
                loss_aux4 = criteria_pre(logits_aux4, lb) * 0.4
                loss_aux.append(loss_aux8)
                loss_aux.append(loss_aux4)
                loss = loss_pre + sum(loss_aux)+loss_aux8+loss_aux4
