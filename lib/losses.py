...
class ContrastCELoss(nn.Module):
    def __init__(self, class_weights, ignore_index=-1, loss_weight=0.1,
                 max_samples=1024, max_views=100, temperature=0.1, base_temperature=0.07):
        super(ContrastCELoss, self).__init__()

        self.loss_weight = loss_weight
        # self.seg_criterion = FSCELoss(class_weights=class_weights, ignore_index=ignore_index)
        self.seg_criterion =OhemCELoss(0.7, ignore_index)
    ...
    def forward(self, preds, target, with_embed=True):
        ...
        # target = target.unsqueeze(1).float().clone()
        # target = F.interpolate(target, (h, w), mode='nearest')
        # target = target.squeeze(1).long()

        # pred = F.interpolate(input=seg, size=(h, w), mode='bilinear', align_corners=True)
        # loss = self.seg_criterion(pred, target)
        # pred = F.interpolate(input=seg, size=(h, w), mode='nearest')
        loss = self.seg_criterion(seg, target)

        pred = F.interpolate(input=seg, size=(h//4, w//4), mode='nearest')
