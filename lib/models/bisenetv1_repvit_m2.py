...
from .repvit_m2 import RepVit_M2
...
class ContextPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.resnet = RepVit_M2()
        self.arm16 = AttentionRefinementModule(320, 256)
        self.arm32 = AttentionRefinementModule(640, 256)
        self.conv_head32 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
        self.conv_head16 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
        self.conv_avg = ConvBNReLU(640, 256, ks=1, stride=1, padding=0)
        self.up32 = nn.Upsample(scale_factor=2.)
        self.up16 = nn.Upsample(scale_factor=2.)

        # replace_batchnorm(self.resnet)
        # self.init_weight()
...
class BiSeNetV1_RepVit_M2(nn.Module):

    def __init__(self, n_classes, aux_mode='train',use_fp16=False, *args, **kwargs):
        super(BiSeNetV1_RepVit_M2, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
