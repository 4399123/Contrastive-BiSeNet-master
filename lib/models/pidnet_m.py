...
class PIDNet(nn.Module):
    ...
    def __init__(self, m=2, n=3, planes=64, ppm_planes=96):
...
class PIDNet_M(nn.Module):
    ...
    def __init__(self, n_classes, aux_mode='train',use_fp16=False, *args, **kwargs):
        ...
        self.conv_out = SegHead(256, 128, n_classes, scale_factor=8)
        if self.aux_mode == 'train':
            self.init_weight()
            self.conv_outp = SegHead(128, 128, n_classes, scale_factor=8)
            self.conv_outd = SegHead(128, 64, n_classes, scale_factor=8)

    def init_weight(self):

        pretrained_state = torch.load('../lib/premodels/PIDNet_M_ImageNet.pth', map_location='cpu')['state_dict']
        # pretrained_state = torch.load('../premodels/PIDNet_M_ImageNet.pth', map_location='cpu')['state_dict']
        model_dict = self.pidnet.state_dict()
        pretrained_state = {k: v for k, v in pretrained_state.items() if (k in model_dict and v.shape == model_dict[k].shape)}
        model_dict.update(pretrained_state)
        logging.info('loading pidnet_m imagesnet checkpoint sucessfully !!!')
